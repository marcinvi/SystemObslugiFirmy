...
namespace Reklamacje_Dane
{
    public partial class WyszukiwarkaZgloszenForm : Form
    {
        ...
        private async Task SynchronizeDataAsync()
        {
            try
            {
                ...
                // *** OPTYMALIZACJA: Sprawdź czy dane są już w cache ***
                if (DataCache.Instance.HasData())
                {
                    ShowLoading(true, "Ładowanie z pamięci cache...");
                    _allData = DataCache.Instance.GetData();
                }
                else
                {
                    // 1. Pobranie danych w tle
                    ShowLoading(true, "Pobieranie z bazy danych...");
                ...
                await Task.Run(() => BuildSideFilters());
        ...
        private void SetupArtisticUI()
        {
            ...
            // *** NOWE: Ustawienie początkowego rozmiaru na prawie pełny ekran ***
        ...
        // *** NOWA METODA: Odświeżanie danych z wymuszeniem pobrania z bazy ***
        private async Task RefreshDataAsync()
        {
            try
            {
                ShowLoading(true, "Wymuszanie synchronizacji z bazą...");
                ...
                ShowLoading(true, "Odbudowa filtrów...");
                await Task.Run(() => BuildSideFilters());
                
                ApplyFilters();
                RepositionColumnFilters();
                
                MessageBox.Show("Dane odświeżone pomyślnie!", "Sukces", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
        ...
        {
            // *** OPTYMALIZACJA: Budowanie wykonane w tle przy ładowaniu ***
        ...
        // --- BRAKUJĄCE EVENT HANDLERY Z DESIGNERA ---
        ...
        {
            // Wyświetl context menu dla dodatkowych filtrów
            // Menu jest już zdefiniowane w Designerze jako contextMenuDodatkoweFiltry
            // i będzie pokazane automatycznie jeśli button ma właściwość ContextMenuStrip
        }
        ...
        {
            // Pokaż context menu z wyborem kolumn
            // Menu jest już zdefiniowane w Designerze jako contextMenuKolumny
        }
        ...
        {
            // Event wywoływany przy zamykaniu menu kolumn
            RepositionColumnFilters();
        }

        private void dataGridViewZgloszenia_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            // Opcjonalnie: obsługa kliknięcia w zawartość komórki
        }

        // --- DODATKOWE FILTRY PREDEFINIOWANE ---

        private void noweReklamacjeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            // Filtr: Reklamacje produktów młodszych niż 3 miesiące
            var threeMonthsAgo = DateTime.Now.AddMonths(-3);
            _filteredData = _allData.Where(x => x.DataZgloszenia.HasValue && x.DataZgloszenia.Value >= threeMonthsAgo).ToList();
            _grid.DataSource = _filteredData;
            _lblStats.Text = $"WYNIKI: {_filteredData.Count} (nowe produkty)";
            _lblStats.ForeColor = Color.Orange;
        }

        private void przeterminowaneToolStripMenuItem_Click(object sender, EventArgs e)
        {
            // Filtr: Zgłoszenia starsze niż 3 dni ze statusem "Nowe"
            var threeDaysAgo = DateTime.Now.AddDays(-3);
            _filteredData = _allData.Where(x => 
                x.DataZgloszenia.HasValue && 
                x.DataZgloszenia.Value <= threeDaysAgo && 
                x.Status == "Nowe"
            ).ToList();
            _grid.DataSource = _filteredData;
            _lblStats.Text = $"WYNIKI: {_filteredData.Count} (przeterminowane)";
            _lblStats.ForeColor = Color.Red;
        }

        private void nierozliczoneNotyToolStripMenuItem_Click(object sender, EventArgs e)
        {
            // Filtr: Nierozliczone noty
            // Zakładam że będą mieć specjalny status
            _filteredData = _allData.Where(x => x.Status != null && x.Status.Contains("nota")).ToList();
            _grid.DataSource = _filteredData;
            _lblStats.Text = $"WYNIKI: {_filteredData.Count} (nierozliczone noty)";
            _lblStats.ForeColor = Color.Purple;
        }

        private void oczekujaceNaDostaweToolStripMenuItem_Click(object sender, EventArgs e)
        {
            // Filtr: Oczekujące na dostawę
            _filteredData = _allData.Where(x => 
                x.Status != null && 
                (x.Status.Contains("oczekuj") || x.Status.Contains("dostaw"))
            ).ToList();
            _grid.DataSource = _filteredData;
            _lblStats.Text = $"WYNIKI: {_filteredData.Count} (oczekujące na dostawę)";
            _lblStats.ForeColor = Color.Blue;
        }

        private void zgloszeniaAllegroToolStripMenuItem_Click(object sender, EventArgs e)
        {
            // Filtr: Tylko zgłoszenia z Allegro
            _filteredData = _allData.Where(x => x.Skad != null && x.Skad.StartsWith("Allegro")).ToList();
            _grid.DataSource = _filteredData;
            _lblStats.Text = $"WYNIKI: {_filteredData.Count} (Allegro)";
            _lblStats.ForeColor = Color.OrangeRed;
        }

        private void wyczyscFiltryDodatkoweToolStripMenuItem_Click(object sender, EventArgs e)
        {
    ...
    // *** NOWA KLASA: Singleton Cache dla danych ***
    public sealed class DataCache
    {
        ...
        public List<ComplaintViewModel> GetData()
        {
            return _cachedData;
        }
