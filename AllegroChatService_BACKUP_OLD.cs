    ...
    newToken.ExpirationDate
        ...
        public async Task MarkMessagesAsReadAsync(IEnumerable<string> messageIds, int userId)
        {
            ...
            try
            {
                using (var con = DatabaseHelper.GetConnection())
                {
                    ...
                    {
                        using (var cmd = new MySqlCommand("INSERT OR IGNORE INTO MessageReadStatus (MessageId, UserId, ReadAt) VALUES (@msgId, @userId, @readAt)", con, transaction))
                        {
        ...
        public async Task<List<ThreadInfo>> GetLatestThreadsAsync()
        {
            ...
            try
            {
                ...
                {
                    await con.OpenAsync();
                    using (var cmd = new MySqlCommand(@"SELECT T.DisputeId, T.MessageText, T.CreatedAt, AD.BuyerLogin, AD.AllegroAccountId, AD.HasNewMessages, AA.AccountName, Z.NrZgloszenia FROM ( SELECT m.DisputeId, m.MessageText, m.CreatedAt, ROW_NUMBER() OVER(PARTITION BY m.DisputeId ORDER BY m.CreatedAt DESC) as rn FROM AllegroChatMessages m ) T JOIN AllegroDisputes AD ON T.DisputeId = AD.DisputeId JOIN AllegroAccounts AA ON AD.AllegroAccountId = AA.Id LEFT JOIN Zgloszenia Z ON AD.ComplaintId = Z.Id WHERE T.rn = 1 ORDER BY T.CreatedAt DESC", con))
                    {
                        using (var reader = await cmd.ExecuteReaderAsync())
                        {
                            while (await reader.ReadAsync())
                            {
                                threads.Add(new ThreadInfo
                                {
                                    ...
                                    LastMessageDate = DateTime.Parse(reader["CreatedAt"].ToString()),
