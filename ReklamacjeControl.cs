...
namespace Reklamacje_Dane
{
    public partial class ReklamacjeControl : UserControl
    {
        ...
        private sealed class SyncServiceStatusLite
        {
        ...
        private sealed class OperationsSyncSnapshotLite
        {
            public SyncServiceStatusLite Google { get; set; } = new SyncServiceStatusLite();
            public SyncServiceStatusLite Dpd { get; set; } = new SyncServiceStatusLite();
        }

        private async Task<OperationsSyncSnapshotLite> GetServerOperationsSyncStatusAsync()
        {
            var snapshot = new OperationsSyncSnapshotLite();
            ...
            try
            {
                ...
                using (var http = new HttpClient())
                {
                    ...
                    http.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", Properties.Settings.Default.ApiToken);
                    var response = await http.GetAsync($"{ApiSyncService.Instance.BaseUrl}/api/ops-sync/status");
                    ...
                    snapshot.Dpd = ParseStatus(data["dpd"] as JObject);
                    snapshot.Google = ParseStatus(data["google"] as JObject);
                    return snapshot;
                }
            }
            catch
        ...
        private static SyncServiceStatusLite ParseStatus(JObject source)
        {
            if (source == null) return new SyncServiceStatusLite();

            return new SyncServiceStatusLite
            {
                LastSuccess = source.Value<bool?>("lastSuccess") ?? false,
                LastError = source.Value<string>("lastError"),
                MetricValue = source.Value<int?>("metricValue") ?? 0
            };
        }
